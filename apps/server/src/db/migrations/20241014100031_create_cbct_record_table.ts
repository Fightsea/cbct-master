...
export async function up(knex: Knex): Promise<void> {
  await knex.schema
    ...
    .createTable('cbct_images', t => {
      ...
      t.uuid('recordId').references('cbct_records.id').notNullable().onDelete('CASCADE');
      t.timestamps(true, true, true);
    });
}
...
export async function down(knex: Knex): Promise<void> {
  await knex.schema
    .dropTable('cbct_images')
    .dropTable('cbct_records');
}
